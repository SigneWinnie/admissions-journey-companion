...
export function Gallery() {
  const { memes, remove, clear } = useGallery();
  ...
  return (
    <section id="gallery" className="mt-16 scroll-mt-20">
      <div className="flex items-end justify-between mb-6">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight">Your gallery</h2>
          <p className="text-sm text-muted-foreground mt-1">
            Memes you saved, stored locally in your browser.
          </p>
        </div>
        {memes.length > 0 && (
          ...
          {memes.map((m) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-2">
                <Button size="icon" variant="secondary" onClick={() => download(m.dataUrl)}>
